    ...
    for radius_mm in core_radius:
        ...
        for n_cores in n_core_list:
            for i in range(iterations):
                ...
                filtered_cores = filtered_points.buffer(radius_pixels)
                ...
                for geom in sampled_gdf:
                    ...
                    if core_nearest_neighbour is not np.nan:
                        core_nearest_neighbour = np.nanmean(
                            core_nearest_neighbour
                        )
                ...
                if not all(np.isnan(nearest_neighbour)):
                    nearest_neighbour_n_mean.append(
                        np.nanmean(nearest_neighbour)
                    )
                    nearest_neighbour_n_stdev.append(
                        np.nanstd(nearest_neighbour)
                    )
                else:
